...
const DamagedReport = ({ setLoading, wrnMsg, scssMsg, errMsg, tab }) =>{
    ...
    const searchAction = async () => {
        ...
        try {
            setLoading(true);
            const res = await fetch(`${import.meta.env.VITE_HOST_LINK}/api/v1/damage/getDamageProductsDetail`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(data)
            })
            const tblData = await res.json();
            console.log("HK data: ", tblData.detailDtoList)
            setTableData(tblData.detailDtoList);
            setLoading(false)
            if (tblData.message === "SUCCESS" && tblData.detailDtoList?.length === 0) wrnMsg("No items found!!!")
            else if (tblData.message === "SUCCESS" && tblData.detailDtoList?.length > 0) scssMsg("Found!!!")
                    ...
                    <div>
                        <div className="relative overflow-x-auto shadow-md sm:rounded-lg">
                            <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                                ...
                                <tbody>
                                    {
                                        subTableData?.detailDtoList?.map(d =>
