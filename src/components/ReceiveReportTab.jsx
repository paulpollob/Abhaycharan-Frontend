...
const ReceiveReportTab = ({ setLoading, wrnMsg, scssMsg, errMsg, tab }) => {
    ...
    const searchAction = async () => {
        ...
        try {
            setLoading(true);
            const res = await fetch(`${import.meta.env.VITE_HOST_LINK}/api/v1/receiveToStore/getReceiveProducts`, {
                method: 'POST',
            ...
            setTableData(tblData.receiveMstDto);
            setLoading(false)
            if (tblData.message === "SUCCESS" && tblData.receiveMstDto?.length === 0) wrnMsg("No items found!!!")
            else if (tblData.message === "SUCCESS" && tblData.receiveMstDto?.length > 0) scssMsg("Found!!!")
    ...
    return (
        <div style={{ transform: `translateX(-${tab * 100}%)` }} className={` transition-transform duration-500 ease-in-out abdsolute s-0  w-1/2 -translate-x-full p-4 min-w-full overflow-hidden bg-gray-50 text-medium text-gray-500 dark:text-gray-400 dark:bg-gray-800 rounded-lg ${(tab == 1) ? ' ' : ' '} `}>
            ...
            <Modal isOpen={isModalOpen} onClose={onCloseModal}>
                ...
                <div ref={printContents}>
                    <Modal.Header>
                        <div className=" w-full flex flex-col justify-center items-center">
                            <h1 className="text-xl font-bold text-white">{subTableData?.storeName}</h1>
                            <p><b>Prop:</b> {subTableData?.distributorName} <b>Mob:</b> {subTableData?.distributorPhone}</p>
                            <span><b>Address: </b>{subTableData?.distributorAddress}</span>
                    ...
                    <div>
                        <div className="relative overflow-x-auto shadow-md sm:rounded-lg">
                            <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                                ...
                                <tbody>
                                    {
                                        subTableData?.productsDetails?.map(d =>
                ...
                <button onClick={(e) => printAction(e)} className="rounded px-4 py-2 bg-slate-900 hover:bg-slate-800 text-white">Print</button>
            </Modal>
        </div>
    )
}
...
export default ReceiveReportTab;